...
exports.messagehandler = async (Infor) => {
  ...
  if (
    !(
      !Infor.isGroup ||
      (Infor.isGroup &&
        Infor.groupdata.totalmsgtoday <= Infor.botdata.dailygrouplimit &&
        (Infor.arg.length !== 0 ||
          (Infor.isGroup && Infor.isMedia && Infor.groupdata.autosticker)))
    )
  )
    return;
  ...
  /* This line checks the following-
    ...
    If yes then return
    */
  if (Infor.isGroup && Infor.groupdata.banned_users.includes(Infor.number))
    return;
  ...
  if (
    Infor.isGroup &&
    Infor.groupdata.membercanusebot === false &&
    !Infor.isGroupAdmins
  )
    return;
  ...
  if (
    ...
    sql.query(
      `UPDATE messagecount SET dailylimitover = true WHERE phonenumber ='${Infor.number}';`
    );
  ...
  if (
    Infor.isGroup &&
    Infor.groupdata.autosticker &&
    Infor.isMedia &&
    Infor.arg[0] !== "sticker" &&
    Infor.arg[0] !== "testnsfw"
  ) {
  ...
  if (
    ...
    Infor.replytext(
      Infor.mess.salutations[
        Math.floor(Math.random() * Infor.mess.salutations.length)
      ]
    );
  ...
  if (commandHandler.has(Infor.arg[0])) {
    if (commandHandler.get(Infor.arg[0]).owner && !Infor.isOwner) {
      ...
      commandHandler.get(Infor.arg[0]).group &&
      Infor.isGroup &&
      !Infor.isGroupAdmins
    ) {
